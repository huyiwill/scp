...
function getAuth($level){
	$auth = array(1=>array('edit_logo'=>false,'hide_ry_logo'=>false,'hide_ry_support'=>false,'exam_rank'=>false,'question_num'=>5,'exam_pub'=>false,'pub_link'=>false),
				  2=>array('edit_logo'=>true,'hide_ry_logo'=>false,'hide_ry_support'=>false,'exam_rank'=>false,'question_num'=>10,'exam_pub'=>1,'pub_link'=>true),
				  ...
				  5=>array('edit_logo'=>true,'hide_ry_logo'=>true,'hide_ry_support'=>true,'exam_rank'=>true,'question_num'=>0,'exam_pub'=>2,'pub_link'=>true));
	return $auth[$level];
}
...
function getTemplateList(){
	$template_list = array(1=>'基础模版',2=>'基础模版+做题反馈',3=>'能力雷达模版',4=>'能力雷达模版+做题反馈','5'=>'百夫长专属(WEB)');
...
function gdata($db,$id,$field='topic',$default='') {
   if ($db!='' && $id!=0 && $field!='') {
       $mydata = Db::name($db)->field($field)->where(array('Id'=>$id))->find();
       if ($mydata) {
	   return $mydata[$field];
	 } else {
	   return $default;
...
function uppic($config = array()){
	...
	$npic    = root().'/public/uploads/default-logo-48.png';
	$isfile  = ($pic!='' && file_exists('./trunk/public/uploads/images/'.$pic)) ? 1 : 0;
    $showpic = ($isfile) ? root().'./trunk/public/uploads/'.$pic  : $npic;
	$cropdata = '';
	$html = '<input id="uppic" name="Filedata" type="file"><input id="pic" name="pic" type="hidden" value="" />';
	$html .= '<div class="layui-box layui-upload-button"><span class="layui-upload-icon"><i class="layui-icon"></i>上传图片</span></div>';
	return '<div class="ui-uploads">'.$html.'</div>'.$cropdata;
}
...
function ganswer($answer,$id) {
  if ( $answer !='' && $id ) {
	$a = '未填';
	foreach($answer as $key=>$val){
	  if ( $key == $id ) {$a = ($val!='') ? $val : '未填';}
	}
	return $a;
  } else {
    return '未填';
